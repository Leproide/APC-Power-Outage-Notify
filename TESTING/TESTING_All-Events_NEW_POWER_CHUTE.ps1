...
function Get-LatestMatchingEvent {
    ...
    try {
        Get-WinEvent -FilterHashtable @{ LogName = $logName; Id = $EventId } -MaxEvents 25 -ErrorAction Stop |
            Where-Object { $_.ProviderName -eq $eventSource } |
...
function Check-EventAndNotify {
    ...
    if (-not $event) {
        Write-Host "No event $EventId found for provider $eventSource."
        return
    }

    if (-not $event.TimeCreated) {
        Write-Host "Event $EventId found but TimeCreated is missing."
        return
    }

    $ageSeconds = ([DateTime]::Now - $event.TimeCreated).TotalSeconds
...
# =========================
$eventsToCheck = $eventMap.Keys | Sort-Object
